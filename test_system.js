...
async function test() {
  try {
    ...
    if (historyRes.data.length > 0) {
      ...
      console.log(
        `   Latest session: ${latest.shots} shots, ${latest.goals} goals`
      );
    }
    ...
    console.log(
      `   Avg accuracy: ${allTimeRes.data.avg_accuracy?.toFixed(2)}%`
    );
    ...
    const leaderboardRes = await axios.get(
      `${API_BASE}/stats/leaderboard?type=global&sort=total_goals&limit=10`,
      {
        headers: { Authorization: `Bearer ${authToken}` },
      }
    );
    ...
    if (leaderboardRes.data.length > 0) {
      console.log(
        `   #1: ${leaderboardRes.data[0].username} (${leaderboardRes.data[0].total_goals} goals)`
      );
    }

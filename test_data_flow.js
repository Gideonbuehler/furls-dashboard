...
async function testDataFlow() {
  ...
  if (!fs.existsSync(statsFile)) {
    ...
    log(
      "ℹ️",
      "Make sure you have played freeplay in Rocket League with the FURLS plugin loaded.",
      COLORS.yellow
    );
    return false;
  }
  ...
  try {
    const currentRes = await axios.get(
      "http://localhost:3002/api/stats/current"
    );
    if (currentRes.data && currentRes.data.shots !== undefined) {
      log("✅", "Backend successfully reading plugin file", COLORS.green);
      log(
        "  ",
        `  Current session: ${currentRes.data.shots} shots, ${currentRes.data.goals} goals`
      );
  ...
  try {
    ...
    if (frontendRes.status === 200 || frontendRes.status === 304) {
      ...
      log(
        "⚠️",
        `Frontend returned status ${frontendRes.status}`,
        COLORS.yellow
      );
    }
  } catch (err) {
    ...
    log(
      "ℹ️",
      "Start the frontend with: cd client && npm run dev",
      COLORS.yellow
    );
    return false;
  }

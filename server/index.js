...
function loadStats() {
  try {
    if (fs.existsSync(STATS_FILE)) {
      ...
      if (
        stats.timestamp &&
        !sessionHistory.some((s) => s.timestamp === stats.timestamp)
      ) {
...
app.get("/api/stats/alltime", (req, res) => {
  const summary = {
    ...
    avgSpeed:
      sessionHistory.length > 0
        ? sessionHistory.reduce((sum, s) => sum + (s.averageSpeed || 0), 0) /
          sessionHistory.length
        : 0,
    totalPlayTime: sessionHistory.reduce(
      (sum, s) => sum + (s.gameTime || 0),
      0
    ),
  };
  res.json(summary);
...
app.listen(PORT, () => {
  ...
  if (process.env.NODE_ENV === "production") {
    console.log(
      `Serving React frontend from: ${path.join(__dirname, "../client/dist")}`
    );
  }
});

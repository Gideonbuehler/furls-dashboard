...
function generateHeatmap(width, height, intensity = 0.3) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      const centerBonus = Math.max(0, 1 - Math.abs(x - width/2) / (width/2)) * 
                         Math.max(0, 1 - Math.abs(y - height/2) / (height/2));
      const value = Math.random() < intensity * (0.5 + centerBonus) 
...
console.log(`   - Accuracy: ${((testData.goals / testData.shots) * 100).toFixed(1)}%`);

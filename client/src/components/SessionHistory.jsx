...
function SessionHistory({ sessionHistory }) {
  ...
  const getAccuracy = (session) => {
    return session.shots > 0 ? ((session.goals / session.shots) * 100).toFixed(1) : 0;
  };
  ...
  return (
    <div className="session-history">
      <h2>📈 Session History</h2>
      <p className="history-info">Showing {sessionHistory.length} most recent sessions</p>
          ...
          <thead>
            ...
            {sessionHistory.slice().reverse().map((session, index) => (
              <tr key={sessionHistory.length - index} className="session-row">
                ...
                <td>
                  <span className={`accuracy ${getAccuracy(session) >= 50 ? 'good' : getAccuracy(session) >= 30 ? 'medium' : 'low'}`}>
                    {getAccuracy(session)}%
                  </span>
                </td>
                <td>{(session.averageSpeed || session.average_speed || 0).toFixed(0)}</td>
                <td>{(session.boostUsed || session.boost_used || 0).toFixed(0)}</td>
                <td>{(session.possessionTime || session.possession_time || 0).toFixed(1)}s</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="summary-grid">
          <div className="summary-item">
            ...
            <span className="summary-value">
              {Math.max(...sessionHistory.map(s => parseFloat(getAccuracy(s)))).toFixed(1)}%
            </span>
          </div>
          <div className="summary-item">
          ...
          <div className="summary-item">
            ...
            <span className="summary-value">
              {Math.max(...sessionHistory.map(s => s.averageSpeed || s.average_speed || 0)).toFixed(0)}
            </span>
          </div>
        </div>
      </div>
